...
$query ="SELECT * 
     ...
     WHERE img.id IN (SELECT ck.id
                  FROM checkstatus 
                  WHERE statuscode = 2) AND ck.statuscode = 2 AND img.zipcode = '$zipcode' ";
...
echo "</Head>";
echo "<Body>";
echo "<p class='mycss'>Accepted Requests</p>";  echo "<br>

<hr> <br>";

echo "<div class='datagrid'><table>
<thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Latitude</th><th>Longitude</th><th>Postal Code</th><th>Street</th><th>City</th><th>Complete Address</th><th>Vehicle Number</th></tr></thead>";
...
//	"</td><td><img src= 'data:image/gif;base64,'" . $data . "/>"
   echo "<tr class='alt'><td>".$row['id']."</td><td>" . $row['name'] . "</td><td>" . $row['email'] . "</td><td>". $row['phone'] . "</td><td>". $row['lat'] . "</td><td>". $row['longitude'] . "</td><td>". $row['zipcode'] . "</td><td>". $row['city'] . "</td><td>". $row['street'] . "</td><td>". $row['fulladddress'] ."</td><td>".$row['vehicle']."</td></tr>";  //$row['index'] the index here is a field name
}
echo "</tbody>";
