...
$query ="SELECT * 
     ...
     WHERE id IN (SELECT id 
                  FROM checkstatus 
                  WHERE statuscode = 0) AND zipcode = '$zipcode' ";
...
.mycss1
{
text-shadow:5px 1px 3px rgba(203,201,255,1);font-weight:normal;color:#000000;letter-spacing:2pt;word-spacing:4pt;font-size:23px;text-align:center;font-family:palatino linotype, palatino, serif;line-height:1;margin:0px;padding:0px;
}
.myButton {
...
echo "<p class='mycss'>Cleaning Requests</p>";  echo "<br>

<hr> <br>";

if(mysql_affected_rows() == 0)
{
echo "<p class = 'mycss1'>No requests to be shown today </p>";
}
else{

echo "<div class='datagrid'><table>
<thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Phone</th><th>Latitude</th><th>Longitude</th><th>Postal Code</th><th>Street</th><th>City</th><th>Complete Address</th><th>Accept Request</th></tr></thead>";
...
//	"</td><td><img src= 'data:image/gif;base64,'" . $data . "/>"
   echo "<tr class='alt'><td>".$row['id']."</td><td>" . $row['name'] . "</td><td>" . $row['email'] . "</td><td>". $row['phone'] . "</td><td>". $row['lat'] . "</td><td>". $row['longitude'] . "</td><td>". $row['zipcode'] . "</td><td>". $row['city'] . "</td><td>". $row['street'] . "</td><td>". $row['fulladddress'] ."</td><td><form method = 'POST' action='confirmrequest.php'><input type='text' id='idname' name='idname' value ='".$row['id']."'><input type='submit' class ='myButton' ></form><hr><form method = 'POST' action='viewimage.php'><input type='text' id='idname' name='idname' value ='".$row['id']."'><input type='submit' value='Image' class ='myButton' ></form></td></tr>";  //$row['index'] the index here is a field name
}
